...
int cnt = jamojamo::count(text, "ㅇ");  // 2 (안녕하세요의 'ㅇ')
int cnt2 = jamojamo::count(text, "ㅏ"); // 1 (안녕하세요의 'ㅏ')
```
...
- 모든 함수는 `std::wstring`과 `std::string` 오버로딩을 지원합니다.
- UTF-8 인코딩의 `std::string`을 사용할 때는 내부적으로 `boost::nowide`를 사용하여 변환합니다.
- 한글 완성형 범위: `0xAC00` (가) ~ `0xD7A3` (힣)
- 자모는 초성 19개, 중성 21개, 종성 28개(없음 포함)입니다.
